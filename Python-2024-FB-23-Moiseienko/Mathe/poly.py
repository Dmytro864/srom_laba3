main file for Mathe lib, implemetation of operation in polinomial basis
import ctypes
import sys
import os
...
lib = ctypes.CDLL(curr_dir + '/Mathe.so')
...
class GFelement:
    ...
    def __pow__(self,n):
        if n == 2:
            ...
            ml = len(mod_D)
            # print(self.words)
            self_D = list(self.words) + [0] * (ml - len(self.words))
            # print("poly.py self.words:",self_D)
            size = len(self_D)
            self_c = (ctypes.c_uint64 * ml)(*self_D)
            res_c = (ctypes.c_uint64 * size)()
            mod_c = (ctypes.c_uint64 * ml)(*mod_D)
            lib.sqr(res_c,self_c,ml,mod_c,ml)
            res_ = list(res_c)
            res = GFelement(res_)
            # print(res)
            #res.reduce()
    ...
    def trace(self):
        ...
        poly_D = POLY.words 
        size = len(poly_D)
        self_D = self.words + [0]*(size - len(self.words))
        assert len(self_D) == len(poly_D) == size 

        self_c = (ctypes.c_uint64 * size)(*self_D)
        poly_c = (ctypes.c_uint64 * size)(*poly_D)
        res_c  = (ctypes.c_uint64 * size)()
        lib.trace(res_c,self_c,poly_c,size)
        res = list(res_c)
        return res[0]
